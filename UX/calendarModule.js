  ...
  */
  ...
  constructor( // calendarClass  client-side
    dom
      ...
      this.tableUx.setStatusLineData( [
        ...
        ,`rows/page: <input type="number" min="1" max="10" size=3 value="${this.tableUx.paging.lines}" onchange="app.calendar.tableUx.changePageSize(this)"/>`
          ...
          // loop for all events for day [m][d]
            ...
            */
            ...
            if (typeof(nodeName) === "string") {
              ...
              html += `<p>${editButton}<a  href="/app.html?p=events&e=${edgeName}&d=${this.format.getISO(start)}${user}" target="_blank" class="${repeat_class}">${this.graph.nodes[nodeName].text[0][2]}</a></p>`
            } else {
  ...
  addNewRepeatMonthy(  // calendarClass  client-side
    ...
    if (this.getOpenMonthDates() <= 3){
      ...
      removeButton.setAttribute('onclick', `app.calendar.removeMonthlySelector(${this.getOpenMonthDates()+1})`)
  ...
  async processServerRefresh( // calendarClass  client-side
    ...
    ,"name":"_graph.json"
    ,"data": this.format.obj2string(app.calendar.graph)
    }
